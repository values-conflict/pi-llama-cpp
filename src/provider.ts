...
import type { LlamaModelInfo } from "./client";
import { llamaInferenceUrl, LlamaClient, normalizeLlamaServerUrl } from "./client";
 ...
 *
 * Key difference from upstream: setCatalog exposes ALL available models, not just loaded ones.
 * This leverages llama.cpp's auto-loading behavior — when a user selects an unloaded model via /model,
 * llama-server will automatically load it before serving the request. Since our hardware only fits
 * one model at a time anyway, this is the natural UX: users see all models and pick freely.
 */
export function createLlamaProvider(): LlamaProviderController {
  ...
  const setCatalog = (catalog: readonly LlamaModelInfo[], serverUrl: string): void => {
    // Show ALL models, not just loaded ones. llama.cpp auto-loads on first request.
